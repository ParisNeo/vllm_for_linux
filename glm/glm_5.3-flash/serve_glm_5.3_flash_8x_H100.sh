...
# H100 (SXM/PCIe) has 80GB HBM3 per GPU vs 141GB HBM3e on the H200.
# Total pool: 8 x 80GB = 640GB. The 0.85 default leaves headroom for the
# CUDA context, activations and the KV cache. If you get OOM during weight
# loading, lower it (e.g. 0.75). If you see a lot of unused memory, raise it
# (up to 0.90).
GPU_MEM_UTIL="${GPU_MEM_UTIL:-0.85}"
MAX_MODEL_LEN="${MAX_MODEL_LEN:-auto}"
# If the KV cache pool ends up smaller than expected on 80GB cards, lower
# this (e.g. 16) to reduce concurrent-sequence memory pressure.
...
Tested Architecture: 8x H100 - Full Tensor Parallel for GLM-5.3-Flash
...
Environment overrides: TP_SIZE, GPU_MEM_UTIL, MAX_MODEL_LEN, MAX_NUM_SEQS,
KV_CACHE_DTYPE, DTYPE, SPEC_METHOD, SPEC_NUM_TOKENS, CUDA_VISIBLE_DEVICES
EOF
}
...
# GLM-5.3 uses specific hybrid attention; the V1 engine handles it.
# If you hit CUDA graph issues on H100, set VLLM_DISABLE_CUDA_GRAPH=1
# when launching this script.
export VLLM_DISABLE_CUDA_GRAPH="${VLLM_DISABLE_CUDA_GRAPH:-0}"

echo "============================================================"
echo " GLM-5.3-Flash vLLM Launcher"
echo " Optimized for 8x H100 (Hopper sm90, 80GB per GPU)"
