...
Tested Architecture: 8x H100 - Full Tensor Parallel for GLM-5.3 Int4-Int8Mix
...
echo " Optimized for 8x H100 (TP=8, ~47GB weights per rank)"
...
if [[ "${SPEC_METHOD}" == "none" || -z "${SPEC_METHOD}" ]]; then
...
for GPU_ID in "${GPU_ARRAY[@]}"; do
  ...
  if [[ ! "${GPU_MEM_FREE}" =~ ^[0-9]+$ ]]; then
    ...
    echo "[PRE-FLIGHT][ERROR] GPU ${GPU_ID} has only ${GPU_MEM_FREE}MiB free (${MIN_FREE_MB}MiB required at TP=${TP_SIZE})."
...
if [[ "${INSUFFICIENT_MEMORY}" -eq 1 ]]; then
  ...
  echo "                 This model holds ~47GB of weights per rank at TP=8."
  echo "                 Run 'nvidia-smi' to identify the process and kill it before retrying,"
  echo "                 or lower MIN_FREE_MB if you run a different TP configuration."
  exit 1
fi
...
if [[ -n "${SPEC_METHOD}" && "${SPEC_METHOD}" != "none" ]]; then
  EXEC_ARGS+=(--speculative-config.method "${SPEC_METHOD}" --speculative-config.num_speculative_tokens "${SPEC_NUM_TOKENS}")
fi
