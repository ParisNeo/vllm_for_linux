...
MODEL_TEXT_VISION="${ROOT_DIR}/models/Qwen__Qwen3.8-Flash-Next-FP8/"
...
echo "============================================================"
echo " 📦 Bundle Launcher: Qwen 3.8 Flash Next + Image Edit + Wan 2.2 Video"
...
if [[ "${SERVE_TEXT}" == true ]]; then
  echo "▶️ Lancement de Qwen 3.8 Flash Next Multimodal (Port 8000)..."
  ...
  echo "   👉 PID : ${PID_TEXT} | Logs : qwen/qwen_3.8_flash_next/vllm_qwen3.8.log"
  sleep 5
fi
...
if [[ "${SERVE_IMAGE}" == true ]]; then
  ...
  echo "   👉 PID : ${PID_IMAGE} | Logs : qwen/qwen_3.8_flash_next/vllm_image_edit.log"
  sleep 5
fi
...
if [[ "${SERVE_VIDEO}" == true ]]; then
  ...
  echo "   👉 PID : ${PID_VIDEO} | Logs : qwen/qwen_3.8_flash_next/vllm_wan_video.log"
  sleep 5
fi
