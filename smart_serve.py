...
- Automatic runner generation on successful startup
...
def create_runner_script(
    ...
    no_multimodal: bool = False,
    no_reasoning: bool = False,
    min_free_gb: float = 10.0,
    max_util: float = 0.90
) -> Path:
    """Generate a runner bash script with all settings baked in."""
    
    # Auto-generate name from model if not provided
    if not name:
        model_name = Path(model).name
        # Clean up name for filesystem
        name = model_name.replace("-", "_").replace(".", "_")
        name = f"{name}_port{port}"
    
    # Ensure runners directory exists
    RUNNERS_DIR.mkdir(parents=True, exist_ok=True)
    
    # Create .gitkeep if it doesn't exist
    gitkeep = RUNNERS_DIR / ".gitkeep"
    if not gitkeep.exists():
        gitkeep.write_text("# This file keeps the runners directory in git\n")
    
    runner_path = RUNNERS_DIR / f"{name}.sh"
    ...
    cmd_args = [
        ...
        f'--port "{port}"',
        f'--gpus "{gpu_str}"',
        f'--max-util "{max_util:.2f}"',
        f'--min-free-gb "{min_free_gb}"',
    ]
    
    if max_model_len:
        cmd_args.append(f'--max-model-len "{max_model_len}"')
    
    if no_multimodal:
        cmd_args.append('--no-multimodal')
    
    if no_reasoning:
        cmd_args.append('--no-reasoning')
...
# GPUs: {gpu_str}
# TP Size: {tp_size}
# GPU Memory Util: {gpu_mem_util:.2f}
# Created: {time.strftime("%Y-%m-%d %H:%M:%S")}
# 
# This runner was auto-generated by smart_serve.py after successful startup.
# All settings are baked in for reproducible deployments.
...
def main():
    ...
    parser.add_argument("--name", type=str, default=None, help="Runner name (for auto-generated runner script)")
    ...
    parser.add_argument("--no-auto-runner", action="store_true", help="Disable automatic runner generation")
    ...
    # Feature control
    parser.add_argument("--no-multimodal", action="store_true", help="Disable multimodal features")
    parser.add_argument("--no-reasoning", action="store_true", help="Disable reasoning parser")
    ...
    # Track the successful configuration for runner generation
    ...
    if not args.no_fallback and not cached_config:
        ...
        for attempt in range(fallback_steps + 1):
            ...
            try:
                # Set environment before exec
                os.environ["CUDA_VISIBLE_DEVICES"] = cuda_visible
                
                # For runner generation, track successful config
                if attempt == 0:  # First attempt is the "optimal" one
                    successful_config = {
                        "tp_size": tp_size,
                        "gpu_mem_util": current_util,
    ...
    # Note: Code below won't execute due to os.execvp, but kept for clarity
    # If we reach here (e.g., in non-exec mode), create runner
    if successful_config and not args.no_auto_runner and not args.dry_run:
        try:
            runner_path = create_runner_script(
                model=args.model,
                port=args.port,
                host=args.host,
                tp_size=successful_config["tp_size"],
                gpu_mem_util=successful_config["gpu_mem_util"],
                gpu_ids=successful_config["gpu_ids"],
                name=args.name,
            ...
            print(f"\n✅ Runner script created: {runner_path}")
            print(f"   Use it with: bash {runner_path}")
        except Exception as e:
            print(f"\n⚠️  Could not create runner script: {e}", file=sys.stderr)
